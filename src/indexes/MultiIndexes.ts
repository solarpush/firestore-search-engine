...
/**
 * Multi-field indexes class for batch processing multiple fields
 * Supports vectorization of multiple fields with weights and custom storage
 */
export class MultiIndexes {
...
/**
 * Enhanced Indexes class with backward compatibility and multi-field support
 */
export class EnhancedIndexes {
  wordMinLength: number;
  wordMaxLength: number;
  ...
  constructor(
    ...
    private readonly fieldValueInstance: typeof firestore.FieldValue,
    private readonly config: FirestoreSearchEngineConfig,
    private readonly props: any // Support both old and new props format
  ) {
    this.wordMinLength = config.wordMinLength || 3;
    this.wordMaxLength = config.wordMaxLength || 50;
  }
  ...
  /**
   * Intelligent indexing that detects single-field vs multi-field configuration
   */
  async smartIndexes(): Promise<void> {
    // Detect if it's multi-field configuration
    if (this.isMultiFieldConfig()) {
      const multiIndexer = new MultiIndexes(
        this.firestoreInstance,
        this.fieldValueInstance,
        this.config,
        this.props as FirestoreSearchEngineMultiIndexesProps
      );
      await multiIndexer.indexes();
    } else {
      // Fallback to single field indexing
      await this.singleFieldIndex();
    }
  }

  /**
   * Check if the configuration is for multi-field indexing
   */
  private isMultiFieldConfig(): boolean {
    return !!(
      this.props.inputFields && typeof this.props.inputFields === "object"
    );
  }

  /**
   * Traditional single-field indexing (backward compatibility)
   */
  private async singleFieldIndex(): Promise<void> {
    const inputField = this.props.inputField;
    if (typeof inputField !== "string") {
      throw new Error(
        "Single field indexing requires inputField to be a string"
      );
    }

    const indexedDocumentPath = this.props.returnedFields.indexedDocumentPath;
    const docRef = this.firestoreInstance.doc(indexedDocumentPath);

    try {
      const cleanText = inputField.toLowerCase().trim();
      if (
        cleanText.length < this.wordMinLength ||
        cleanText.length > this.wordMaxLength
      ) {
        console.warn(
          `⚠️ Texte trop court/long pour l'indexation: ${cleanText.length} caractères`
        );
        return;
      }

      const vector = await fse_vectorizeText(cleanText, this.wordMaxLength);

      const indexDocument = {
        ...this.props.returnedFields,
        _vector: vector,
        fieldValue: cleanText,
        _indexed_at: this.fieldValueInstance.serverTimestamp(),
      };

      await docRef.set(indexDocument, { merge: true });
      console.log(`✅ Index simple créé: ${indexedDocumentPath}`);
    } catch (error) {
      console.error(
        `❌ Erreur lors de l'indexation simple de ${indexedDocumentPath}:`,
        error
      );
      throw error;
    }
