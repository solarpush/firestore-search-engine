...
import { addresses } from "./datasets/addresses";
...
describe("cosine similarity", () => {
  describe("Embedding model precision for model_quint8_avx2", () => {
    it("should give higher similarity for semantically close sentences", async () => {
      ...
      const sims: number[] = [];
      const gold: number[] = [];
      for (const { a, b, score } of addresses) {
      ...
      console.log(
        "Addresses Average error for model_quint8_avx2 :",
        avgError.toFixed(3)
      );
      expect(avgError).toBeLessThan(0.4); // seuil à ajuster selon ton modèle
    });
  });
  ...
  describe("Embedding model precision for BGESmallEN", () => {
    it("should give higher similarity for semantically close sentences", async () => {
      ...
      console.log(
        "Addresses Average error for BGESmallEN:",
        avgError.toFixed(3)
      );
      expect(avgError).toBeLessThan(0.4); // seuil à ajuster selon ton modèle
    });
  ...
  describe("Embedding model precision for BGEBaseENV15", () => {
    it("should give higher similarity for semantically close sentences", async () => {
      ...
      console.log(
        "Addresses Average error for BGEBaseENV15:",
        avgError.toFixed(3)
      );
      expect(avgError).toBeLessThan(0.4); // seuil à ajuster selon ton modèle
    });
